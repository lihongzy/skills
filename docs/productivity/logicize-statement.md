...
The skill treats everything after the command as source text. Its logical rewrite makes relationships between ideas explicit and repairs language errors, while retaining the same content, scope, and intent. It does not remove, merge, summarise, infer, or add information.

## Common questions

**Does it add missing details or remove incomplete ones?**

No. It repairs expression and logic only. Information that is absent, unclear, or incomplete in the source remains that way in the result.

**Does it handle the task described by the input?**

No. It only rewrites the supplied text. It does not analyse, answer, or carry out its contents.
